...
def _parse_label(label: str) -> str:
    if not isinstance(label, str):
        raise TypeError(
            f"'{label!s}' is of type {type(label)!s}, which is not an accepted type."
...
def _parse_value(value: Value) -> str:
    ...
    raise TypeError(
        f"'{value!s}' is of type {type(value)!s}, which is not an accepted type."
...
def _parse_delta(delta: Delta) -> str:
    ...
    if isinstance(delta, str):
        ...
        raise TypeError(
            f"'{delta!s}' is of type {type(delta)!s}, which is not an accepted type."
...
def _determine_delta_color_and_direction(
    ...
    if delta_color not in {"normal", "inverse", "off"}:
        raise StreamlitAPIException(
            f"'{delta_color!s}' is not an accepted value. delta_color only accepts: "
